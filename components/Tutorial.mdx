...
5. 5 - [Enrichment analysis with ChEA-KG](#chea-enrichment)
	1. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1 - [Perform a query](#perform-query)
	2. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2 - [Filter the results](#filter-query)
	3. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3 - [View results in a table or bar chart](#view-table)
...
To perform the search, click on the node name in the menu. Alternatively, search either of the two example nodes by clicking on their names under "Example". 
...
<img src="https://minio.dev.maayanlab.cloud/hgrn-chear/tutorial/two-term.png" alt="drawing" width="800"style="border: 1px solid grey;" /> 
...
*Expand the CTCFL node by clicking expand (Figure 8):*  
...
<p>&nbsp;</p>
*Delete the expanded CTCFL node by clicking delete (Figure 9):*  
...
**Figure 9.** Remove the CTCFL node by clicking delete (top). This removes the CTCFL node and its relationships (bottom).  
  
**Network and Table View:**  
In addition to the network view, there is also the option to view the network as a table. To change between the network and table view, click on their respective buttons (Figure 10). Three tables are produced, one for the nodes and one for each edge type,
...
**Figure 10.** View the network as a table using the Table View button.  
...
**Figure 11.** Switch to a hierarchical layout using the Graph Layout button. 

**Show Edge Labels:**  
To overlay the edge labels ("upregulates" and "downregulates") on the graph edges, click on the Edge Labels button. (Figure 12).
...
**Figure 12.** Display the edge labels using the Edge Labels button.  

**Show Legend:**  
To display a legend describing the node and edge colors, use the Show Legend button (Figure 13). On clicking this button, an additional toolbar button is displayed that, when clicked, adjusts the size of the legend.  
<img src="https://minio.dev.maayanlab.cloud/hgrn-chear/tutorial/legend.png" alt="drawing" width="800"style="border: 1px solid grey;" /> 
**Figure 13.** Show and adjust the size of the legend. 
...
**Figure 14.** The "Enrichment" tab landing page.  

This feature generates a subnetwork of the GRN containing the TFs with the highest integrated mean rank for that gene set. By default, the top 10 ranked TFs that appear in at least 3 out of 6 ChEA3 primary libraries are included. The ChEA3 integrated mean rank is calculated from the average rank across six libraries: Enrichr Queries, GTEx Coexpression, ARCHS4 Coexpression, ENCODE ChIP-seq, Literature ChIP-seq, and ReMap ChIP-seq. For more information on how the ChEA3 Integrated Mean Rank is calculated, visit the [ChEA3 website](https://maayanlab.cloud/chea3/). 
...
**Figure 15**. Performing a ChEA-KG query using the example gene set.  
  
Click "submit" to view the resulting TF network (Figure 16). The toolbar at the top containing options to reformat and 
...
**Figure 16.** Subnetwork of enriched TFs for the example gene set. Yellow nodes are enriched TFs that are also present in the input gene set.  
  
### *Filter query results and add more nodes* *<a name="filter-query"></a>
...
**Example**  
Adjust "Add nodes" to 15 and click the send icon ("Filter subnetwork"). The network now shows the top 25 TFs for the gene set (Figure 17).  
...
**Figure 17.** Add 15 more top-ranked nodes by adjusting the "Add nodes" slider (top). The network now displays the top 25 nodes (bottom). 
...
up to the top 25 TFs and edges with a zscore of more than 10. Only nodes with significant edges are kept (Figure 18).  
...
**Figure 18.** Filter out edges with a zscore of less than 10 by adjusting the slider (top). The network now contains only nodes with significant edges (bottom).

### *View results in a table or bar chart* <a name="view-table"></a>
...
The score indicates how relevant the gene set is to the enriched TF, with a lower score indicating higher relevancy. "Overlapping genes" is the 
number of genes shared between the input gene set and the TF's annotated targets (Figure 19). The bar view shows the enriched TFs ordered by their score.
Mousing over each bar displays a tooltip with the score and number of overlapping genes for that TF (Figure 20). 
...
**Figure 19.** View the results as a table. 
...
**Figure 20.** View the results as a bar chart. 